...
[EMODnet](https://emodnet.ec.europa.eu/en/emodnet-web-service-documentation) provides two primary types of OGC-compliant geospatial web services, each designed for a specific spatial data type:
...
**In R:** Use the [`emodnet.wfs`](https://docs.ropensci.org/emodnet.wfs/) package to access WFS services. The package simplifies discovering available layers, filtering by extent or attributes, and retrieving data as `sf` objects.
...
**In R:** Use the [`emodnet.wcs`](https://emodnet.github.io/emodnet.wcs) package to access WCS services. The package handles coverage discovery, spatial/temporal subsetting, and returns data as `terra` raster objects.
...
### Finding Authoritative Product Documentation

The metadata exposed by WFS `GetCapabilities` and WCS `DescribeCoverage` (titles, abstracts, attributes) is publisher-dependent: sometimes it carries full product descriptions, often it carries only a short summary. For the authoritative, scientifically detailed account of an EMODnet data product (methodology, formulas, source datasets, units, version history), the canonical references live elsewhere:

-   **[EMODnet Data Product Catalogue (Geonetwork)](https://emodnet.ec.europa.eu/geonetwork/srv/eng/catalog.search)** -- Each EMODnet data product has a metadata record describing how it was built and where the underlying data came from. Searchable by keyword, organisation, and theme.
-   **GitHub product repositories** -- Many EMODnet products (especially in Biology and Seabed Habitats) publish their full processing code openly under the [EMODnet GitHub organisation](https://github.com/EMODnet). These contain the methods, parameters, and references behind the gridded products.
-   **EMODnet thematic service pages** -- [Biology](https://emodnet.ec.europa.eu/en/biology), [Bathymetry](https://emodnet.ec.europa.eu/en/bathymetry), [Seabed Habitats](https://emodnet.ec.europa.eu/en/seabed-habitats), [Human Activities](https://emodnet.ec.europa.eu/en/human-activities), etc., each maintain product index and news pages with user-facing descriptions.

When working with a layer or coverage in R, start with the in-API metadata for quick orientation; then consult the catalogue or product repository for definitions you can rely on for analysis.
...
::: callout-note
## Specifying CRS: EPSG Codes and WKT
...
- Website: [proj.org](https://proj.org/)
...
- Website: [libgeos.org](https://libgeos.org/)
...
- Website: [s2geometry.io](https://s2geometry.io/)
...
1. **Loading data**: GDAL reads the file format
2. **Reprojecting**: PROJ transforms coordinates
3. **Spatial operations**: GEOS or s2 performs intersections, buffers, etc.
4. **Saving results**: GDAL writes the output format
...
The [`sf` package](https://r-spatial.github.io/sf/) is the modern standard for **vector spatial data** in R. It provides an R interface to GDAL (for data I/O), PROJ (for CRS transformations), and GEOS/s2 (for geometric operations).
