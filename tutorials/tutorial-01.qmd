...
| Layer | Description |
|-------|-------------|
| `cddaareas` | CDDA (Common Database on Designated Areas) — European inventory of nationally designated protected areas including nature reserves, national parks, and other site types |
| `natura2000areas` | Natura 2000 network — EU-wide network of protected areas established under the Birds and Habitats Directives to protect Europe's most valuable species and habitats |
...
| Layer | Description |
|-------|-------------|
| `platforms` | Offshore platforms — oil, gas, and wind energy installations including production platforms, substations, and support structures |
| `pipelines` | Subsea pipelines — oil, gas, and chemical transport pipelines on the seabed |
...
We'll use `emodnet.wfs` to access EMODnet WFS services, `sf` for spatial data handling, `dplyr` and `purrr` for data manipulation, and `ggplot2` and `tmap` for visualisation.
...
```r
install.packages(c("emodnet.wfs", "sf", "dplyr", "purrr", "ggplot2", "tmap"))
```
...
We'll focus on the North Sea, defined by the following [bounding box](../concepts.qmd#bounding-boxes-and-spatial-queries):
...
We'll use [EPSG:3035](../concepts.qmd#coordinate-reference-systems-crs) (ETRS89-LAEA) as our working CRS—a projected coordinate system with metric units that's well-suited for European marine analysis.
...
ggplot() +
  ...
  geom_sf(data = bbox_3035, fill = NA, color = "#0077BE", linewidth = 1.5) +
  labs(x = NULL, y = NULL) +
  theme_minimal() +
...
The `layer_attribute_descriptions()` function returns metadata about each attribute—name, type, and whether it's the geometry column. Let's check all three protected area layers:
...
mpa_descriptions <- map(
  ...
  ~ layer_attribute_descriptions(wfs = wfs, layer = .x)
)
...
get_geom_cols <- function(descriptions) {
  map_chr(descriptions, ~ .x$name[.x$geometry])
}
...
Looking at these descriptions, we can identify character attributes that might contain categorical values useful for filtering:

- **`cddaareas`**: `majorecosy` (ecosystem type) and `designated` (designation status) look promising
- **`natura2000areas`**: `mar_perc` (marine percentage) could help us select primarily marine sites
- **`marineprotectedareas`**: `status` likely indicates whether sites are designated or proposed

#### Exploring Attribute Values

To check our assumptions, we can use `layer_attribute_inspect()` to fetch the unique values for specific attributes. This is much cheaper than downloading the full dataset:

```{r}
#| label: inspect-filter-candidates
#| cache: true
...
This confirms our filter strategy:
...
Now that we understand what's available, we can plan exactly what to request. Defining a schema upfront has several benefits:
...
mpa_property_names <- map2_chr(
  mpa_schemas,
  mpa_geom_cols,
  ~ get_property_names(schema = .x, geom_col = .y)
)
mpa_property_names
```
...
# Define Irish Sea exclusion zone (in EPSG:4326, then transform)
...
# Remove features that intersect with the Irish Sea
mpa_sf_list <- lapply(mpa_sf_list, function(x) {
  in_irish_sea <- lengths(st_intersects(x, irish_sea_bbox)) > 0
  x[!in_irish_sea, ]
})
...
- **Client-side**: You download the data first, then filter it locally in R using functions like `st_intersects()`. This is what we did above with the Irish Sea exclusion.
...
Server-side filtering reduces download size, which matters for large datasets. Client-side filtering with sf functions like `st_intersects()` is more flexible and works with any spatial data—not just WFS—making it a valuable skill for spatial analysis in R.
:::
...
imap(infra_descriptions, ~ .x$name)
```
...
infra_sf_list <- lapply(infra_sf_list, function(x) {
  in_irish_sea <- lengths(st_intersects(x, irish_sea_bbox)) > 0
  x[!in_irish_sea, ]
})
...
sapply(infra_sf_list, function(x) unique(st_geometry_type(x)))
```
...
purrr::map_chr(mpa_sf_list, ~ st_crs(.x)[["input"]])

# Check CRS of infrastructure
purrr::map_chr(infra_sf_list, ~ st_crs(.x)[["input"]])
```
...
sapply(mpa_sf_list, function(x) sum(!st_is_valid(x)))
```

```{r}
#| label: fix-protected-area-geometries
# Fix any invalid geometries
mpa_sf_list <- lapply(mpa_sf_list, st_make_valid)
```
...
sapply(infra_sf_list, function(x) sum(!st_is_valid(x)))
```

```{r}
#| label: fix-infrastructure-geometries
infra_sf_list <- lapply(infra_sf_list, st_make_valid)
```
...
lapply(mpa_sf_list, names)
```

To combine them with `bind_rows()`, we need to standardise these first. We'll use the schema we defined in [Planning Our Data Schema](#planning-our-data-schema), where the `from` field maps source column names to our target names (e.g., `name` → `sitename`).
...
standardise_sf <- function(data, schema = NULL) {
  ...
  data |>
    ...
    mutate(across(any_of(names(schema)), ~ get_type(cur_column())(.x)))
}
```
...
infra_by_type <- list(
  ...
  cables = bind_rows(infra_sf_list[grep("^pcables", names(infra_sf_list))]),
  platforms = infra_sf_list$platforms
)

sapply(infra_by_type, nrow)
```
...
|----------|---------|----------|
| `st_intersects()` | Logical matrix | Which features touch or overlap? |
| `st_intersection()` | New geometries | What is the actual overlapping area? |
| `st_within()` | Logical matrix | Which features are completely inside? |
| `st_contains()` | Logical matrix | Which features completely contain others? |
...
intersections <- map(
  infra_by_type,
  ~ lengths(st_intersects(mpa_combined, .x)) > 0
) |>
  bind_cols() |>
...
To visualise the results, we need to combine the intersection flags with the MPA geometries. We also simplify the geometries—marine protected area boundaries often contain highly detailed coastlines with thousands of vertices, which slows down plotting.
