...
All data comes from **EMODnet Human Activities**:
...
| [`natura2000areas`](https://emodnet.ec.europa.eu/geonetwork/srv/api/records/ac911c34-1692-4642-a031-87bcb5822158) | Natura 2000 network, the EU-wide network of protected areas established under the Birds and Habitats Directives to protect Europe's most valuable species and habitats |
| [`marineprotectedareas`](https://emodnet.ec.europa.eu/geonetwork/srv/api/records/f727edd7-87b8-4b02-b173-14ca13025f6f) | Other marine protected areas, sites designated under regional sea conventions (OSPAR, HELCOM, Barcelona Convention) and other international frameworks |
...
| [`pipelines`](https://emodnet.ec.europa.eu/geonetwork/srv/api/records/aca3dd01-77ac-47fe-8291-6ca916daaa6d) | Subsea pipelines for oil, gas, and chemical transport on the seabed |
| [`pcablesbshcontis`](https://emodnet.ec.europa.eu/geonetwork/srv/api/records/41b339f8-b29c-4550-b787-3d68f08fdbcc) | Power cables from BSH (German Federal Maritime and Hydrographic Agency) and CONTIS database |
| [`pcablesshom`](https://emodnet.ec.europa.eu/geonetwork/srv/api/records/41b339f8-b29c-4550-b787-3d68f08fdbcc) | Power cables from SHOM (French Naval Hydrographic and Oceanographic Service) |
| [`pcablesrijks`](https://emodnet.ec.europa.eu/geonetwork/srv/api/records/41b339f8-b29c-4550-b787-3d68f08fdbcc) | Power cables from Rijkswaterstaat (Dutch Directorate-General for Public Works and Water Management) |
| [`pcablesnve`](https://emodnet.ec.europa.eu/geonetwork/srv/api/records/41b339f8-b29c-4550-b787-3d68f08fdbcc) | Power cables from Norwegian sources |
...
We'll use [`emodnet.wfs`](https://emodnet.github.io/emodnet.wfs/) to access EMODnet WFS services, [`sf`](https://r-spatial.github.io/sf/) for spatial data handling, [`rnaturalearth`](http://docs.ropensci.org/rnaturalearth/) to get European coastlines, [`dplyr`](https://dplyr.tidyverse.org/) and [`purrr`](https://purrr.tidyverse.org/) for data manipulation, and [`ggplot2`](https://ggplot2.tidyverse.org/) and [`tmap`](https://r-tmap.github.io/tmap/) for visualisation.
...
EMODnet provides data through several thematic services. Let's see what [WFS services](../concepts.qmd#wfs-web-feature-service) are available:
...
To visualise the results, we need to combine the intersection flags with the MPA geometries. We also simplify the geometries because marine protected area boundaries often contain highly detailed coastlines with thousands of vertices, which slows down plotting. The same applies to the pipeline and cable layers, which are typically digitised at very high resolution and can carry hundreds of thousands of coordinate points.
...
# Simplify pipeline and cable geometries for the same reason
infra_by_type$pipelines <- st_simplify(
  infra_by_type$pipelines,
  dTolerance = 50, preserveTopology = TRUE
)
infra_by_type$cables <- st_simplify(
  infra_by_type$cables,
  dTolerance = 50, preserveTopology = TRUE
)
...
The `dTolerance` parameter specifies the simplification tolerance in CRS units. Since we're using EPSG:3035 (metric), `dTolerance = 2000` means 2 km for the MPA polygons (imperceptible at the map scale but dramatically faster to render) and `dTolerance = 50` means 50 m for the pipeline and cable lines, fine enough to keep their shape recognisable at deep zoom but coarse enough to drop the bulk of redundant sub-metre digitisation.
...
tmap_save(interactive_map, get_output_path("interactive_map.html", "tutorials/maps"))

knitr::include_url("maps/interactive_map.html", height = "600px")
```
...
- [EMODnet Seabed Habitats](https://emodnet.ec.europa.eu/en/seabed-habitats): Explore more protected area and habitat data
- [EMODnet Human Activities](https://emodnet.ec.europa.eu/en/human-activities): Access additional infrastructure datasets
