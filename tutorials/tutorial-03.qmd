...
# The in-memory list-of-rasters + lazy merge pattern used in the
# visible chunk fails on the GitHub runner (tile tempfiles get
# garbage collected between chunks under memory pressure). Here we
# write each tile to a named .tif, merge to another named .tif, and
# remove the tile files once the merge has materialised.
x_breaks <- seq(study_bbox["xmin"], study_bbox["xmax"], length.out = 7)
y_breaks <- seq(study_bbox["ymin"], study_bbox["ymax"], length.out = 6)
...
for (i in seq_len(n_x)) {
  for (j in seq_len(n_y)) {
    ...
    tile_files[k] <- tempfile(fileext = ".tif")
    terra::writeRaster(
      emdn_get_coverage(
        wcs = bathy_wcs,
        coverage_id = bathy_coverage_id,
...
# Keep tile_files on disk for the rest of the session; they'll be cleaned
# up automatically when R exits. Removing them here breaks downstream reads
# if `bathymetry` still holds lazy references to them.
```
